...
class LHS implements ParserConstants, Serializable {
    ...
    */
    ...
    private Object getValueImpl() throws UtilEvalError
    {
        ...
        if ( type == FIELD ) try {
            Invocable inv = Objects.requireNonNull(field,
                "get value, field cannot be null");
            Class<?> declClass = isStatic() ? inv.getDeclaringClass() : object.getClass();
            JavaFieldHookParam fieldHook = new JavaFieldHookParam(
                declClass, inv.getName(), inv.getReturnType(),
            ...
            if (!fieldHook.isIntercepted()) {
                Object result = inv.invoke(object);
                fieldHook.setReturnValue(result);
            }
    ...
    */
        ...
        throws UtilEvalError
    {
        if ( type == VARIABLE )
        {
            ...
            Class<?> declClass = isStatic() ? field.getDeclaringClass() : object.getClass();
            JavaFieldHookParam fieldHook = new JavaFieldHookParam(
                declClass, field.getName(), field.getReturnType(),
                val, null, JavaFieldHookParam.ACCESS_WRITE,
                false, Thread.currentThread().getStackTrace());
            Interpreter.bshHookManager.fireBeforeJavaField(fieldHook);
            if (!fieldHook.isIntercepted()) {
                Object finalVal = fieldHook.getValue();
                Objects.requireNonNull(field,
                    "assign value, field cannot be null").invoke( object, finalVal);
            }
            Interpreter.bshHookManager.fireAfterJavaField(fieldHook);
            return getValueImpl();
