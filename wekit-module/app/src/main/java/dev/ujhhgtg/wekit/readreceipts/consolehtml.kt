...
object ConsoleHtml {
    ...
    private fun ts(epoch: Long): String {
        return dateFormat.format(Date(epoch * 1000))
    }
    ...
    private fun fmtLoc(loc: String?): String {
        ...
        try {
            ...
            val lonDir = if (lon >= 0) "东经" else "西经"
    ...
    fun index(db: Database): String {
        val stats = db.stats()
        val tm = stats[0]
        val tr = stats[1]
        val rows = db.messageListAsc(200)
        ...
        sb.append("<!DOCTYPE html><html lang=\"zh-CN\"><head><meta charset=\"UTF-8\">")
          ...
          .append("<div class=\"readcount\">总已读 <b>").append(tr.toString()).append("</b> 次</div>")
        ...
        if (rows.count == 0) {
            ...
            while (rows.moveToNext()) {
                ...
                val wxId = rows.getString(rows.getColumnIndexOrThrow("wx_id"))
                val content = rows.getString(rows.getColumnIndexOrThrow("content"))
                val cnt = rows.getInt(rows.getColumnIndexOrThrow("cnt"))
                val reg = rows.getLong(rows.getColumnIndexOrThrow("registered_at"))
                sb.append("<div class=\"msg-item\" id=\"msg-").append(id).append("\">")
                  ...
                  .append("<span class=\"msg-read\">").append(cnt.toString()).append(" 人已读</span>")
                  .append("</div>")
                  .append("</div>")
            }
        }
    ...
    fun detail(db: Database, msg: Cursor): String {
        val id = msg.getString(msg.getColumnIndexOrThrow("id"))
        val wxId = msg.getString(msg.getColumnIndexOrThrow("wx_id"))
        val content = msg.getString(msg.getColumnIndexOrThrow("content"))
        val reg = msg.getLong(msg.getColumnIndexOrThrow("registered_at"))
        val reads = db.readList(id)
        ...
        sb.append("<!DOCTYPE html><html lang=\"zh-CN\"><head><meta charset=\"UTF-8\">")
          ...
          .append("</div>")
          // 已读记录
          .append("<div class=\"section-title\">已读记录 (").append(reads.count.toString()).append(")</div>")

        if (reads.count == 0) {
            sb.append("<div class=\"empty\">📭 暂无读取记录</div>")
        } else {
            while (reads.moveToNext()) {
                var reader = reads.getString(reads.getColumnIndexOrThrow("reader_wx_id"))
                if (reader == null || reader.isEmpty()) {
                    reader = reads.getString(reads.getColumnIndexOrThrow("wx_id"))
                }
                val ip = reads.getString(reads.getColumnIndexOrThrow("ip_address"))
                val country = reads.getString(reads.getColumnIndexOrThrow("country"))
                val region = reads.getString(reads.getColumnIndexOrThrow("region"))
                val city = reads.getString(reads.getColumnIndexOrThrow("city"))
                val isp = reads.getString(reads.getColumnIndexOrThrow("isp"))
                val loc = fmtLoc(reads.getString(reads.getColumnIndexOrThrow("loc")))
                val rt = reads.getLong(reads.getColumnIndexOrThrow("read_at"))

                val addr = StringBuilder()
                if (!country.isNullOrEmpty()) addr.append(country).append(" ")
                if (!region.isNullOrEmpty()) addr.append(region).append(" ")
                if (!city.isNullOrEmpty()) addr.append(city)
                val addrStr = if (addr.isNotEmpty()) addr.toString() else "-"

                sb.append("<div class=\"read-card\">")
                  .append("<div class=\"read-ip\">").append(esc(ip)).append("</div>")
                  .append("<div class=\"read-row\"><b>地址</b>").append(esc(addrStr)).append("</div>")
                  .append("<div class=\"read-row\"><b>运营商</b>").append(esc(isp ?: "-")).append("</div>")
                  .append("<div class=\"read-row\"><b>经纬度</b>").append(esc(loc ?: "-")).append("</div>")
